import { signIn } from "next-auth/react";
import { useState } from "react";

import type { OAuthProviderOption } from "~/server/oauth";

const PROVIDER_LABELS: Record<OAuthProviderOption["id"], string> = {
  google: "Google",
  github: "GitHub",
};

type OAuthButtonsProps = {
  providers: OAuthProviderOption[];
  callbackUrl?: string;
};

/**
 * Social sign-in / sign-up buttons. OAuth creates an account on first use, so
 * the same control works on both `/auth/signin` and `/auth/signup`.
 */
export function OAuthButtons({
  providers,
  callbackUrl = "/dashboard",
}: OAuthButtonsProps) {
  const [configHint, setConfigHint] = useState<string | null>(null);
  const anyConfigured = providers.some((p) => p.configured);

  return (
    <div className="mt-6">
      <div className="relative">
        <div className="absolute inset-0 flex items-center" aria-hidden>
          <div className="w-full border-t border-slate-200/80" />
        </div>
        <div className="relative flex justify-center text-xs uppercase tracking-wide">
          <span className="bg-white/80 px-2 text-slate-500 backdrop-blur-sm">
            Or continue with
          </span>
        </div>
      </div>

      <ul className="mt-4 space-y-2" role="list">
        {providers.map(({ id, configured }) => (
          <li key={id}>
            <button
              type="button"
              disabled={!configured}
              title={
                configured
                  ? undefined
                  : `Add ${id === "google" ? "GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET" : "AUTH_GITHUB_ID and AUTH_GITHUB_SECRET"} to .env, then redeploy`
              }
              onClick={() => {
                if (!configured) {
                  setConfigHint(
                    id === "google"
                      ? "Google sign-in needs GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET in your .env file. Restart the dev server after saving."
                      : "GitHub sign-in needs AUTH_GITHUB_ID and AUTH_GITHUB_SECRET in .env (and in AWS via redeploy).",
                  );
                  return;
                }
                setConfigHint(null);
                void signIn(id, { callbackUrl });
              }}
              className={`oauth-btn oauth-btn-${id} w-full disabled:cursor-not-allowed disabled:opacity-50`}
            >
              <OAuthIcon provider={id} />
              <span>{PROVIDER_LABELS[id]}</span>
            </button>
          </li>
        ))}
      </ul>

      {configHint && (
        <p className="mt-3 rounded-md bg-amber-50 px-3 py-2 text-sm text-amber-800 ring-1 ring-amber-200">
          {configHint}
        </p>
      )}

      {!anyConfigured && (
        <p className="mt-3 text-center text-xs text-slate-500">
          Buttons are disabled until OAuth keys are added to{" "}
          <code className="rounded bg-slate-100 px-1">.env</code>. See{" "}
          <code className="rounded bg-slate-100 px-1">.env.example</code> for
          variable names.
        </p>
      )}
    </div>
  );
}

function OAuthIcon({ provider }: { provider: OAuthProviderOption["id"] }) {
  if (provider === "google") {
    return (
      <svg className="h-5 w-5" viewBox="0 0 24 24" aria-hidden>
        <path
          fill="#4285F4"
          d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
        />
        <path
          fill="#34A853"
          d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        />
        <path
          fill="#FBBC05"
          d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"
        />
        <path
          fill="#EA4335"
          d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        />
      </svg>
    );
  }

  return (
    <svg className="h-5 w-5 text-slate-900" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      <path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0 0 24 12c0-6.63-5.37-12-12-12z" />
    </svg>
  );
}
